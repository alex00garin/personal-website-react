          ...
          <div className="py-8 lg:py-12 px-4 mx-auto max-w-screen-md">
            ...
            <form action="#" id="contactForm" className="space-y-8 justify-center">
                <div>
                    ...
                    <input onChange={(event)=>setName(event.target.value)} type="text" id="name" autoComplete="name" className="font-rajdhani block p-3 w-full text-sm text-black bg-neutral-200 rounded-lg border border-gray-300 shadow-sm focus:ring-1 focus:outline-none focus:ring-black" placeholder="Write your name here" required />
                </div>
                <div>
                    <label htmlFor="email" className="block mb-2 text-sm font-medium text-black dark:text-white">Your email*</label>
                    <input onChange={(event)=>setEmail(event.target.value)} type="email" id="email" autoComplete="email" className="block p-3 w-full text-sm text-black bg-neutral-200 rounded-lg border border-gray-300 shadow-sm focus:ring-1 focus:outline-none focus:ring-black" placeholder="your@email.com" required />
                </div>
                <div className="sm:col-span-2">
                    <label htmlFor="message" className="block mb-2 text-sm font-medium text-black dark:text-white">Your message*</label>
                    <textarea onChange={(event)=>setMessage(event.target.value)} id="message" rows="6" className="block p-3 w-full text-sm text-black bg-neutral-200 rounded-lg border border-gray-300 shadow-sm focus:ring-1 focus:outline-none focus:ring-black" placeholder="Leave a comment..." required ></textarea>
                </div>

                <div className="flex justify-center">
                <button onClick={handleSubmit} disabled={submissionStatus === 'Sending...'} id="submitBtn" type="submit" className="inline-flex mt-5 py-2.5 px-5 mr-2 gap-3 text-sm font-medium border border-1 rounded-lg text-black dark:text-white border-black dark:border-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-bg ease-in-out duration-500">{submissionStatus || 'Send message'}</button>
                </div>                                               
            </form>
          </div>
        </section>
